...
#program = ["./test_program/heapsort","./test_program/mergesort"]
...
class mythread (threading.Thread):
    ...
    def run(self):
        ...
        #a = [self.ins , self.pro , self.p , str(self.page) , str(self.tlb)]
        sort = subprocess.Popen(['valgrind', '--tool=lackey', '--trace-mem=yes', str(self.pro), str(self.ins)]
                                , stderr=subprocess.STDOUT, stdout=subprocess.PIPE)

        # f = open(a, "w")
        base = ['./valtlb379', '-p', str(self.poli), str(self.page), str(self.tlb)]
        # if self.i:
        #     base.append('-i')
        ...
        #data.update(str(self.poli)+" "+str(self.page)+" "+str(self.tlb))
...
if __name__ == "__main__":
    ...
    for key,value in filedata.items():
        ...
        with open("./tmp.plt","w") as f:
            ...
            f.write('''plot \
newhistogram "FIFO", \
'tmp.dat' using "F512":xtic(1) t col, '' u "F4096" t col, '' u "F32768" t col, \
newhistogram "LRU", \
'tmp.dat' using "L512":xtic(1) t col, '' u "L4096" t col, '' u "L32768" t col

set output
set term pop''')
        subprocess.Popen(['gnuplot',"tmp.plt"])
        print("----------------------------------")